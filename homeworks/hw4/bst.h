...
/// Swap left
template<typename KeyT, typename ValueT>
void BSTNode<KeyT,ValueT>::swapLeft()
{
...
/// Swap right
template<typename KeyT, typename ValueT>
void BSTNode<KeyT,ValueT>::swapRight()
{
...
/// Detach from parent
template<typename KeyT, typename ValueT>
void BSTNode<KeyT,ValueT>::detachFromParent() 
{
    BSTNode<KeyT,ValueT>* p = parent();
    if (p == nullptr) {
        return;
...
BSTNode<KeyT,ValueT>* BST<KeyT,ValueT>::remove(KeyT aKey)
{
    ...
    if (n->left() != nullptr &&
        ...
        n->setKey(s->key());
        (void) trivialRemove(s);
        return parentNode;
    } else {
        // Cases 1, 2 and 3
        return trivialRemove(n);
    }
    return nullptr;
}
